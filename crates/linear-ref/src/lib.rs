...
impl Drop for Prepared {
    fn drop(&mut self) {
        unsafe {
            ffi::miz23_graph_delete(self.graph);
        };
    }
}
...
impl Drop for Computed {
    fn drop(&mut self) {
        unsafe {
            ffi::miz23_result_delete(self.result);
        };
    }
}
    ...
    pub fn finalize(&self) -> DiGraph<MDNodeKind, ()> {
        ...
        for node in &nodes {
            match convert_to_node_kind(node) {
                ...
                Err(NodeKindConversion::Removed) => {
                    panic!("node was removed");
                }
                Err(NodeKindConversion::OutOfRange) => {
                    panic!("node kind value is not valid");
                }
            }
        }

        let edges = nodes.iter().enumerate().filter_map(|(idx, node)| {
...
mod test {
    ...
    fn test0() {
        ...
        let count_node_kinds = |tree: &DiGraph<MDNodeKind, ()>| -> (usize, usize, usize, usize) {
            tree.node_references().fold((0, 0, 0, 0), |(prime, series, parallel, vertex), (_, k)| match k {

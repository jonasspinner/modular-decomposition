...
impl Graph {
    ...
    pub(crate) fn edge_references(&self) -> impl Iterator<Item = (NodeIndex, NodeIndex, EdgeIndex)> + '_ {
        self.node_indices().flat_map(|u| self.incident_edges(u).map(move |(v, e)| (u, v, e)))
    }
        ...
        I: IntoIterator<Item = EdgeIndex> + Clone,
    {
        let move_edge = |x: NodeIndex, xy: EdgeIndex, nodes: &mut [Node], edges: &mut [Edge]| {
            ...
            if let Some(e) =
                edges[start.index()..end.index()].iter().rposition(|e| !e.deleted).map(|i| (start.index() + i).into())
            {
                if xy < e {
    ...
    fn find_edge(&self, u: NodeIndex, v: NodeIndex) -> Option<EdgeIndex> {
        ...
        self.edges[start.index()..end.index()]
            .iter()
            .position(|e| e.head == v)
            .map(|pos| EdgeIndex::new(start.index() + pos))
    }
        ...
        for u in 0..self.nodes.len() - 1 {
            if !(self.nodes[u].start.0 <= self.nodes[u].end.0 && self.nodes[u].end.0 <= self.nodes[u + 1].start.0) {
                panic!(
                    "u={}  [{} {}] [{} ..]",
                    u,
                    self.nodes[u].start.0,
                    self.nodes[u].end.0,
                    self.nodes[u + 1].start.0
                );
            }
...
mod test {
    ...
    fn remove_edges() {
        ...
        println!(
            "{:?}",
            graph.nodes.iter().enumerate().map(|(i, n)| (i, n.start.index(), n.end.index())).collect::<Vec<_>>()
        );
        println!(
            "{:?}",
            graph
                ...
                .iter()
                .enumerate()
                .map(|(i, e)| (i, graph.edges[e.twin.index()].head.index(), e.head.index(), e.twin.index()))
                .collect::<Vec<_>>()
        );

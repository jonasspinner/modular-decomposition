...
use crate::algos::utils::{
    add_all_nodes_as_children, build_quotient, check_series_parallel, determine_spine_node_kind, try_trivial_quotient,
};
...
fn chain(graph: &mut Graph, v: NodeIndex, tree: &mut Tree, current: TreeNodeIndex) {
    // Compute the modular decomposition of a nested graph with {v} being an
    // innermost vertex in it.
...
fn rop(graph: &mut Graph, V: Part, V_idx: TreeNodeIndex, partition: &mut Partition, tree: &mut Tree) {
    ...
    if try_single_vertex(&V, V_idx, partition, tree) {
        return;
    }
...
pub(crate) mod tree {
    ...
    impl Tree {
        ...
        pub(crate) fn into_vec(self) -> Vec<TreeNode> {
            self.nodes
        }
    }
...
mod utils {
    use crate::algos::tree::Kind::{Parallel, Prime, Series, Vertex};
    ...
    #[allow(non_snake_case)]
    pub(crate) fn try_trivial_quotient(
        crossing_edges: &[(NodeIndex, NodeIndex)],
        p: SubPartition,
        partition: &Partition,
        current: TreeNodeIndex,
        tree: &mut Tree,
    ) -> Option<(Part, TreeNodeIndex)> {
        ...
        assert!(
            B.len() > 1,
            "The case |X| = 2 should be covered earlier. |X| > 2 and |P(G[X], v)| = 2 imply |p1| = |X - {{v}}| > 1."
        );
        Some((B, B_idx))
    }

    #[allow(non_snake_case)]
    pub(crate) fn build_quotient(
        crossing_edges: &mut Vec<(NodeIndex, NodeIndex)>,
        X: SubPartition,
        partition: &Partition,
        inner_vertex: NodeIndex,
        map: &mut Vec<NodeIndex>,
    ...
    #[allow(non_snake_case)]
    pub(crate) fn add_all_nodes_as_children(
        X: Part,
        X_idx: TreeNodeIndex,
        kind: Kind,
        partition: &Partition,
...
mod test {
    ...
    fn ted08_test0_v0_quotient() {
        let graph = Graph::from_edges(
            5,
            [(0, 1), (1, 2), (2, 3), (3, 4), (1, 3)].map(|(u, v)| (NodeIndex::new(u), NodeIndex::new(v))),
        );

...
impl FastSet {
    ...
    pub fn set(&mut self, x: impl Into<usize>) {
        self.data[x.into()] = self.generation;
    }
    pub fn reset(&mut self, x: impl Into<usize>) {
        self.data[x.into()] = u32::MAX
    }
    pub fn get(&self, x: impl Into<usize>) -> bool {
        self.data[x.into()] == self.generation
    }

    pub fn items(&self) -> impl Iterator<Item = usize> + '_ {
        self.data.iter().enumerate().flat_map(|(i, &gen)| (gen == self.generation).then_some(i))
    }
}

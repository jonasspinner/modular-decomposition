...
impl Computed {
    pub fn finalize(&self) -> DiGraph<MDNodeKind, ()> {
        ...
        let add_node =
            |forest: &Forest<MDComputeNode>, tree: &mut DiGraph<MDNodeKind, ()>, u| -> NodeIndex<DefaultIx> {
                let kind: MDNodeKind = {
                    let data = &forest[u].data;
                    match data.node_type {
                        NodeType::Vertex => MDNodeKind::Vertex(data.vertex.idx()),
                        NodeType::Operation => match data.op_type {
                            Operation::Prime => MDNodeKind::Prime,
                            Operation::Series => MDNodeKind::Series,
                            Operation::Parallel => MDNodeKind::Parallel,
                        },
                        NodeType::Problem => {
                            panic!("problem nodes should be non existent")
                        }
                    }
                };
                tree.add_node(kind)

...
impl<Data> Node<Data> {
    ...
    pub fn is_first_child(&self) -> bool {
        self.has_parent() && self.left.is_none()
    }
    pub fn is_last_child(&self) -> bool {
        self.has_parent() && self.right.is_none()
    }
...
impl<Data> Forest<Data> {
    ...
    pub fn is_valid(&self, index: NodeIdx) -> bool {
        index.idx() < self.capacity()
    }
...
impl<Data> Forest<Data> {
    ...
    pub fn swap(&mut self, a: NodeIdx, b: NodeIdx) {
        ...
        let mut update_adj = |x, y| -> (Option<NodeIdx>, Option<NodeIdx>, Option<NodeIdx>) {
            ...
            if let Some(left) = left {
                self[left].right = Some(y);
            }
            if let Some(right) = right {
                self[right].left = Some(y);
            }
            (parent, left, right)
        };

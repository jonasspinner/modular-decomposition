...
impl Debug for MDComputeNode {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self.node_type {
            NodeType::Vertex => {
                write!(f, "{}", self.vertex.idx())
            }
            NodeType::Operation => match self.op_type {
                Operation::Prime => {
                    write!(f, "P")
                }
                Operation::Series => {
        ...
        while let Some(current_problem) = tree.as_valid(current_problem_) {
            ...
            if !tree[child].data.is_problem_node() {
                ...
                tree.subtree_node_mut(tree[current_problem].first_child.unwrap(), |node| {
                    if node.is_leaf() {
                        alpha_list[node.data.vertex.idx()].clear();
                    }
                    node.data.clear();
                });
            ...
            current_problem_ = if tree[current_problem].is_last_child() {
                tree[current_problem].parent
            } else {
                tree[current_problem].right
            };
        }

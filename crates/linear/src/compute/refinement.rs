...
mod set_up {
    ...
    pub(crate) fn number_by_tree(tree: &mut Forest<MDComputeNode>, problem: NodeIdx) {
        for (tree_number, c) in tree.children(problem).collect::<Vec<_>>().into_iter().enumerate() {
            tree.subtree_node_mut(c, |node| {
                node.data.tree_number = tree_number as _;
            });
        }
    }
}
...
mod marking_split_types {
    ...
    pub(crate) fn mark_ancestors_by_split(
        tree: &mut Forest<MDComputeNode>,
        index: NodeIdx,
        split_type: SplitDirection,
    ) {
        ...
        loop {
            let p_ = p.unwrap();
            if tree[p_].data.is_problem_node() {
                break;
            }
    ...
    pub(crate) fn get_max_subtrees(tree: &mut Forest<MDComputeNode>, leaves: &[NodeIdx]) -> Vec<NodeIdx> {
        ...
        for x in full_charged {
            if !is_parent_fully_charged(tree, x) {
                ret.push(x);
            }
        }
        for x in charged {
            tree[x].data.clear_marks();
        }
        ret
    }
}
...
fn refine_one_node(tree: &mut Forest<MDComputeNode>, index: NodeIdx, split_type: SplitDirection, new_prime: bool) {
    trace!(
        "refining tree={}, index={}, split_type={:?}, new_prime={}",
        tree.to_string(Some(index)),
        index.idx(),
        split_type,
        new_prime as i32
    );
    if is_root_operator(tree, index) {
        return;
    }
    ...
    if is_root_operator(tree, p) {
        ...
        for st in [SplitDirection::Left, SplitDirection::Right] {
            ...
            if tree[p].data.is_split_marked(st) {
                tree[replacement].data.increment_num_split_children(st)
            }
        }
    }
...
fn refine_with(tree: &mut Forest<MDComputeNode>, alpha_list: &[Vec<NodeIdx>], refiner: NodeIdx, pivot: NodeIdx) {
    ...
    trace!(
        "sibling_groups: {:?}, tree={}",
        sibling_groups.iter().map(|(n, b)| (n.idx(), *b as i32)).collect::<Vec<_>>(),
        tree.to_string(tree[NodeIdx::from(pivot.idx())].parent)
    );

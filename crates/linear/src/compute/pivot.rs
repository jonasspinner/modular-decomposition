...
fn is_pivot_layer(tree: &Forest<MDComputeNode>, index: NodeIdx) -> bool {
    ...
    if let Some(parent) = node.parent {
        ...
        parent.data.is_problem_node() && Some(NodeIdx::from(parent.data.vertex.idx())) == node.first_child
    } else {
        false
    }
}
...
fn pull_forward(tree: &mut Forest<MDComputeNode>, v: NodeIdx) {
    ...
    if tree[current_layer].data.connected {
        return;
    }
    ...
    trace!(
        "tree[prev_layer].data.active={}, is_pivot_layer()={}",
        tree[prev_layer].data.active as i32,
        is_pivot_layer(tree, prev_layer) as i32
    );
    ...
    if tree[prev_layer].data.connected {
        tree.move_to(v, prev_layer);
    }
    if tree[current_layer].is_leaf() {
...
pub(crate) fn process_neighbors(
    graph: &Graph,
    tree: &mut Forest<MDComputeNode>,
    alpha_list: &mut [Vec<NodeIdx>],
    visited: &[bool],
    pivot: VertexId,
    current_problem: NodeIdx,
    neighbor_problem: Option<NodeIdx>,
) {
...
pub(crate) fn do_pivot(
    ...
    if tree[current_problem].is_leaf() {
        tree.remove(current_problem);
    }

    if tree[neighbor_problem].is_leaf() {
        tree.remove(neighbor_problem);
    }

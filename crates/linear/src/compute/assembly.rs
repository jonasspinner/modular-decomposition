...
fn compute_fact_perm_edges(
    tree: &mut Forest<MDComputeNode>,
    alpha_list: &[Vec<NodeIdx>],
    ps: &[NodeIdx],
    pivot_index: usize,
    vset: &mut FastSet,
    fp_neighbors: &mut [Vec<VertexId>],
) {
...
fn assemble_tree(
    ...
    while 0 <= lb || rb < k {
        ...
        tree[new_module].data.op_type = match (added_neighbors, added_non_neighbors) {
...
pub(crate) fn assemble(
    ...
    for p in tree.children(prob) {
        if p.idx() == current_pivot.idx() {
            pivot_index = ps.len() as _
        }
        ps.push(p);
    }

...
impl Partition {
    ...
    fn check_node(nodes: &[Node], idx: u32) -> bool {
        ...
        if left != u32::MAX && nodes[left as usize].next != idx {
            return false;
        }
        if right != u32::MAX && nodes[right as usize].prev != idx {
            return false;
        }
        true
    }
    ...
    fn insert_before(nodes: &mut [Node], a: u32, b: u32) {
        ...
        if a_prev != u32::MAX {
            nodes[a_prev as usize].next = a_next;
        }
        if a_next != u32::MAX {
            nodes[a_next as usize].prev = a_prev;
        }

        nodes[a as usize].prev = b_prev;
        if b_prev != u32::MAX {
            nodes[b_prev as usize].next = a;
        }
    ...
    fn new_part(&mut self, first: u32, prev: u32, next: u32, gen: u32) -> u32 {
        ...
        if prev != u32::MAX {
            self.parts[prev as usize].next = idx;
        } else {
            self.first_part = idx;
        }
        if next != u32::MAX {
            self.parts[next as usize].prev = idx;
        }
        idx
    }
    ...
    fn remove_part(&mut self, idx: u32) {
        ...
        if prev != u32::MAX {
            self.parts[prev as usize].next = next;
        } else {
            self.first_part = next;
        }
        if next != u32::MAX {
            self.parts[next as usize].prev = prev;
        }
    ...
    pub(crate) fn refine(&mut self, pivots: impl IntoIterator<Item = u32>) {
        self.gen += 1;
        let is_appendable = |parts: &[Part], gen: u32, idx: u32| idx < u32::MAX && parts[idx as usize].gen == gen;
        ...
        for pivot in pivots {
            ...
            let pivot_part_is_singleton =
                pivot_is_first && (node_next == u32::MAX || self.nodes[node_next as usize].part != part);
            ...
            if pivot_part_is_singleton {
                ...
                // Append to the previous part if it is appendable (cases 4 and 6), otherwise
                // create a new part for the pivot (cases 3 and 5).
                self.nodes[pivot as usize].part =
                    if prev_part_is_appendable { prev } else { self.new_part(pivot, prev, part, self.gen) };
                if pivot_is_first {
                    // 3:  [.. a]' [x b ..]' -> [.. a]' [x]* [b ..]'
                    // 4:  [.. a]* [x b ..]' -> [.. a    x]* [b ..]'
            ...
            debug_assert_eq!(
                !pivot_part_is_singleton,
                is_appendable(&self.parts, self.gen, self.nodes[pivot as usize].part)
            );
...
impl<'a> AllElementsIterator<'a> {
    fn new(partition: &'a Partition) -> Self {
        Self {
            ...
            next: partition.parts[partition.first_part as usize].first,
            len: partition.nodes.len() as _,
        }
    }
}
...
impl ExactSizeIterator for AllElementsIterator<'_> {
    fn len(&self) -> usize {
        self.len as _
    }
}
...
#[allow(dead_code)]
impl Partition {
    pub(crate) fn elements(&self) -> AllElementsIterator {
        AllElementsIterator::new(self)
    }
}
...
impl Iterator for PartElementsIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.next == u32::MAX || self.nodes[self.next as usize].part != self.part {
            return None;
        }
        let current = self.next;
        self.next = self.nodes[current as usize].next;
...
impl ExactSizeIterator for PartsIterator<'_> {
    fn len(&self) -> usize {
        self.len as _
    }
}

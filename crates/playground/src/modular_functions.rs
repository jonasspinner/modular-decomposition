...
impl MDTree {
    ...
    /// The minimal strong module containing two vertices x and y.
    ///
    /// Also denoted by *m(x,y)*.
    ...
    fn vertices(&self, x: NodeIndex) -> Vec<NodeIndex> {
        if let MDNodeKind::Vertex(u) = self.md_tree[x] {
            return vec![NodeIndex::new(u)];
        }
        let mut dfs = Dfs::new(&self.md_tree, x);
        let mut module = vec![];
        while let Some(c) = dfs.next(&self.md_tree) {
            if let MDNodeKind::Vertex(u) = self.md_tree[c] {
                module.push(NodeIndex::new(u))
            }
        }
        module.sort();
        module
    ...
    /// The maximal strong module containing x but not y, for any two different
    /// vertices x, y of G.
    ///
    /// Also denoted by *M(x, y)*.
...
#[cfg(test)]
mod test {
    use crate::modular_functions::MDTree;
    ...
    fn v_modular_partition_test() {
        ...
        assert_eq!(
            ...
            .iter()
            .map(|v| v.iter().map(|&i| NodeIndex::<DefaultIx>::new(i as _)).collect::<Vec<_>>())
            .collect::<Vec<_>>()
        );
    }

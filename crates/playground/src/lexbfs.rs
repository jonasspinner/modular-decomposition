mod rou21 {
    ...
    use petgraph::graph::{node_index, IndexType, UnGraph};
        ...
        Ix: IndexType,
    {
        ...
        let mut vertices_info: Vec<_> = (0..graph.node_count())
            .map(|i| VertexInfo { order: i as _, interval: intervals.handle_front().unwrap(), reached: false })
            .collect();
        ...
        for i in 0..ordering.len() {
            ...
            for neighbor in neighbors {
                ...
                let start_order = std::mem::replace(
                    &mut vertices_info[ordering[intervals[interval].start as usize] as usize].order,
                    order,
                );

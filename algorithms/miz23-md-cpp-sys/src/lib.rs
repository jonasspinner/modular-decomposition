pub mod ffi {
    ...
    extern "C" {
        pub fn miz23_modular_decomposition(n: c_int, edges: *const Edge, m: c_int, out_result: *mut *mut Result) -> c_int;
...
mod tests {
    ...
    fn it_works() {
        let num_vertices = 5;
        let edges = [(0, 1), (1, 2), (2, 3), (3, 4), (1, 3)].map(|(u, v)| Edge{ u, v });

        let mut result = null_mut();
        let status = unsafe { miz23_modular_decomposition(num_vertices as _, edges.as_ptr(), edges.len() as _, &mut result as *mut *mut _) };
        assert_eq!(status, 0);

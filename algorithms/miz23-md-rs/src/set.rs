...
impl FastSet {
    ...
    pub(crate) fn set(&mut self, x: impl Into<usize>) { self.data[x.into()] = self.generation; }
    fn reset(&mut self, x: impl Into<usize>) { self.data[x.into()] = u32::MAX }
    pub(crate) fn get(&self, x: impl Into<usize>) -> bool { self.data[x.into()] == self.generation }

    fn items(&self) -> impl Iterator<Item=usize> + '_ {

...
impl ListHandle {
    pub(crate) fn index(&self) -> usize { self.0 as _ }
}

impl TryFrom<InnerHandle> for ListHandle {
    type Error = ();
    fn try_from(handle: InnerHandle) -> Result<Self, Self::Error> {
        if handle.is_valid() { Ok(ListHandle(handle.0)) } else { Err(()) }
    }
}

impl From<ListHandle> for InnerHandle {
    fn from(handle: ListHandle) -> Self { Self(handle.0) }
}
...
impl<T> Default for List<T> {
    fn default() -> Self {
        Self { nodes: Vec::default(), removed: Vec::default(), first: InnerHandle::invalid(), last: InnerHandle::invalid() }
    }
}
...
impl<T> List<T> {
    ...
    pub(crate) fn is_empty(&self) -> bool { !self.first.is_valid() }

    pub(crate) fn len(&self) -> usize { self.nodes.len() - self.removed.len() }
    ...
    pub(crate) fn handle_front(&self) -> Option<ListHandle> { self.first.try_into().ok() }
    pub(crate) fn front(&self) -> Option<&T> { self.get_node(self.first).map(|f| &f.value) }
    pub(crate) fn front_mut(&mut self) -> Option<&mut T> { self.get_node_mut(self.first).map(|f| &mut f.value) }
    pub(crate) fn handle_back(&self) -> Option<ListHandle> { self.last.try_into().ok() }
    pub(crate) fn back(&self) -> Option<&T> { self.get_node(self.last).map(|f| &f.value) }
    pub(crate) fn back_mut(&mut self) -> Option<&mut T> { self.get_node_mut(self.last).map(|f| &mut f.value) }
    ...
    pub(crate) fn push_front(&mut self, value: T) {
        ...
        self.get_node_mut(old_first).map(|n| {
            n.prev = handle;
        });
        self.first = handle;
        if !self.last.is_valid() { self.last = handle; }
    }
    pub(crate) fn push_back(&mut self, value: T) {
        ...
        let handle = handle.into();
        self.get_node_mut(old_last).map(|n| {
            n.next = handle;
        });
        self.last = handle;
        if !self.first.is_valid() { self.first = handle; }
    }
}
...
impl<T> List<T> {
    ...
    pub(crate) fn pop_front(&mut self) {
        match TryInto::<ListHandle>::try_into(self.first) {
            Ok(first) => self.remove(first),
            _ => {}
        }
    }
    pub(crate) fn pop_back(&mut self) {
        match TryInto::<ListHandle>::try_into(self.last) {
            Ok(last) => self.remove(last),
            _ => {}
        }
    }
}
...
impl<T> List<T> {
    fn iter(&self) -> Iter<'_, T> { Iter::new(self) }
}

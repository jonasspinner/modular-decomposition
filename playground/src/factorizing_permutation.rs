...
pub mod kar19_checked_impl {
    ...
    pub fn factorizing_permutation(graph: &UnGraph<(), ()>) -> Vec<NodeIndex> {
        ...
        let mut state = State { P, center, pivots, modules, first_pivot, _positions, _nodes, _parts, _removed, _gen, _pivots, _modules, _first_pivot, _center_pos, _non_singleton_idx };
    ...
    impl State {
        ...
        fn _debug_print(&self) {
            println!("#       P: {:?}", self.P.iter().map(|X| X.iter().map(|u| u.index()).collect::<Vec<_>>()).collect::<Vec<_>>());
            ...
            println!("_   parts: {:?}", self._parts.iter().map(|s| (s.seq.first.index(), s.seq.len, s.gen.0)).collect::<Vec<_>>());
            println!("#  pivots: {:?}", self.pivots.iter().map(|X| X.iter().map(|u| u.index()).collect::<Vec<_>>()).collect::<Vec<_>>());
            //println!("_  pivots: {:?}", self._pivots.iter().map(|s| (s.first.index(), s.len)).collect::<Vec<_>>());
            println!("_  pivots: {:?}", self._pivots.iter().map(|X| X.range().map(|pos| self._nodes[pos].node.index()).collect::<Vec<_>>()).collect::<Vec<_>>());
            println!("# modules: {:?}", self.modules.iter().map(|X| X.iter().map(|u| u.index()).collect::<Vec<_>>()).collect::<Vec<_>>());
            println!("_ modules: {:?}", self._modules.iter().map(|X| X.range().map(|pos| self._nodes[pos].node.index()).collect::<Vec<_>>()).collect::<Vec<_>>());
            //println!("_ modules: {:?}", self._modules.iter().map(|s| (s.first.index(), s.len)).collect::<Vec<_>>())
        }
            ...
            if false {
                ...
                while i < self._nodes.len() {
                    ...
                    if between {
                        ...
                        self._parts[X_a_idx.index()].seq = Seq { first: NodePos::new(X.first.index() + X_len as usize), len: X.len - X_len };
                    } else {
                        ...
                        self._parts[X_a_idx.index()].seq.len = (j - self._parts[X_a_idx.index()].seq.first.index()) as u32;
                        self._parts[X_idx.index()].seq = Seq { first: NodePos::new(j), len: X.len - self._parts[X_a_idx.index()].seq.len };
                    }
                ...
                for u in graph.neighbors(y) {
                    ...
                    if _E.contains(u_pos) { continue; }

                    let X = self.part(self.node(u_pos).part).seq;
                    if X.contains(self._center_pos) || X.contains(y_pos) { continue; }
        ...
        fn _insert_right(&mut self, u_pos: NodePos) {
            ...
            let next = (last.index() + 1 != self._nodes.len()).then(|| {
                self._nodes[last.index() + 1].part
        ...
        fn _insert_left(&mut self, u_pos: NodePos) {
            ...
            let prev = (first.index() != 0).then(|| {
                self._nodes[first.index() - 1].part
        ...
        fn part(&self, idx: PartIndex) -> &Part { &self._parts[idx.index()] }
        fn part_mut(&mut self, idx: PartIndex) -> &mut Part { &mut self._parts[idx.index()] }
        fn node(&self, idx: NodePos) -> &Node { &self._nodes[idx.index()] }
        fn position(&self, node: NodeIndex) -> NodePos { self._positions[node.index()] }
            ...
            while self.init_partition(graph) {
                while let Some(E) = self.pivots.pop() {
                    ...
                    for &x in &E {
                        ...
                        let _S: HashSet<_> = graph.neighbors(x).filter(|v| !_E.range().contains(&self._positions[v.index()].index())).collect();
                        assert_eq!(S, _S);
        ...
        fn init_partition(&mut self, graph: &UnGraph<(), ()>) -> bool {
            if self.next_non_singleton().is_none() { return false; }
            ...
            if let Some(X) = self.modules.pop_front() {
                ...
                if true {
                    ...
                    self._parts[N_idx.index()].seq.len = (w_pos.index() - self._parts[N_idx.index()].seq.first.index()) as u32;
    ...
    mod test {
        ...
        fn graph_002() {
            let graph = UnGraph::from_edges([
                (0, 1), (0, 3), (0, 4), (1, 3), (1, 4), (2, 3), (2, 4), (3, 5), (4, 5), (5, 6), (5, 7), (6, 7), (8, 11), (9, 11), (10, 11)
            ]);
